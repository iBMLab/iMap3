function imap3(cfg)
%% 
% iMap toolbox for eye-tracking data analysis - Version 3
% http://perso.unifr.ch/roberto.caldara/index.php?page=4
% Sébastien Miellet, Cyril R. Pernet, Junpeng Lao, Guillaume A. Rousselet, Luca Vizioli, & Roberto Caldara (2013). University of Fribourg
...
% 1. Update details
% version 3
% 1- New "statistical engine" for iMap3. iMap3 now uses t-tests and bootstrapped TFCE transformed scores to correct for multiple comparisons (TFCE: threshold-free cluster-enhancement; Smith & Nichols, 2009), instead of being based on the Random Field Theory.
...
%    We are also grateful to Chris McManus for the careful reading of the original version of iMap, which stimulated us to improve our toolbox.
% 2- A bug due to the impossibility of calculating effect sizes when there is no effect has been fixed.
% 3- This version includes an option to generate the raw fixations maps.
...
% 
% 3. How to use iMap3
% Copy imap3 and its support functions in the folder containing the data file (or alternatively set paths for input and output data).
...
% Some analyses with small stimulus size can be performed on a 4 Go RAM computer. However, we strongly recommend using a 8 or even better 16 Go RAM computer.
% We recommend using iMap at first with the default settings to have a general view on the results. Then, in a second step for the final analysis, we advise to use a higher number of bootstraps (1000 or more for better estimate) and, if necessary, to set the color scale and transparency of the maps.
% 
...
% 17-	mindatapoints: Minimal number of data points required to include a pixel in the analysis (default = 10). Obviously, iMap3 cannot compute the pixel-wise t-values without (enough) data. Hence, in order to obtain reliable variability and central tendency indices, a minimal number of data points per pixel is necessary. Please see the face processing example for details on this variable.
% 18-	logicalmask: Generates tiff images (called logicalmask1, logicalmask2 and logicalmaskcontrast) representing the pixels included in the analysis in function of the number of data points threshold. 1 = no (default), 2 = yes
% 
% 5. Output
...
% 2-  Global eye-tracking measures. iMap creates .txt files with global eye-tracking measures for both datasets (called eyebasicdataset1.txt and eyebasicdataset2.txt). The columns are: the number of fixations, the total fixation duration, the mean fixation duration, the path length and the mean saccade length). The lines correspond to the raw data files (participants, sessions).
...
% 4-  Effect sizes: It also creates a .txt file with the cohen's d between both datasets for area 1 and 2 (areas in which the fixation durations are significantly longer for dataset 1 and 2 respectively). The file is called cohend.txt. The same data set is used to both determine the significant areas with iMap and compute the effect sizes from these significant areas (for selection and selective analysis). Henceforth, the non-independent selective analysis might distort descriptive statistics (see Kriegeskorte, Simmons, Bellgowan & Baker, 2009; example 2). It is relatively straightforward to generate the distribution of independent random split-data effect sizes computed with a resampling procedure. This option is not implemented by default in iMap as it requires a very long processing time.
% 5-  Eye-tracking measures in significant areas: imap creates .txt files with the eye-tracking data in both the significant areas and the rest of the picture. The files are called eyeareadataset1.txt and eyeareadataset2.txt and are organised the following way: mean fixation duration for area 1 then for area 2 then for the rest of the picture. In the same way are path length, total fixation duration and number of fixations. Please refer to the code for the exact output structure that might vary depending on the observed significant effects. 
% 6- Raw maps: Raw (without normalization and smoothing) fixation locations and durations for each data set. The raw maps are generated by default.
...
% Eye movement data were analysed with iMap3 (Caldara and Miellet, 2011), which implements a novel statistical approach to correct for multiple comparisons (Pernet, Chauveau, Gaspar, Rousselet, 2011; Smith and Nichols, 2009).
% 
% References:
...
if isfield(cfg,'searchspace')
    ...
    if ischar(searchspace)
        masksearchspace = double(imread(searchspace));
        pixsearchspace = (masksearchspace - min(masksearchspace(:))) / (max(masksearchspace(:)) - min(masksearchspace(:)));
    else
        error('please indicate the file name for the search space');
    end
end
...
if isfield(cfg,'mindatapoints')
    if cfg.mindatapoints<2
        error('mindatapoints needs to be at the very least 2');
    else
...
if isfield(cfg,'logicalmask') 
    logicalmask=cfg.logicalmask;
else
    logicalmask=1; % 1 = no (default), 2 = yes
end
...
clear summary datatoload nbfixsbj pathlengthsbj cumulfixdur meansacclength cumulsaccadelength dataset eyebasicdataset1 eyebasicdataset2 datasetname datasetnb datafilenumber meanfixdur meanfixdursbj meansacclentgh meansacclengthsbj nbfix nbfixtrial nbvariables numfix pathlength totalfixdur totalfixdursbj trialnb

%% Descriptive fixation maps
matrixdurationtotal=zeros(length(datatotal),xSize, ySize);
smoothpictotal=zeros(length(datatotal),xSize, ySize);
mask = NaN(length(datatotal),xSize,ySize);
for datafilenumber=1:length(datatotal)
    ...
    matrixdurationtotal(datafilenumber,:,:)=matrixduration;
    % SMOOTHING step
    ...
    tmp=tmp-min(tmp(:));
    tmp(tmp<0.05)=0;
    smoothpictotal(datafilenumber,:,:)=tmp;
    mask(datafilenumber,:,:) = smoothpictotal(datafilenumber,:,:) > 0.05;
end

% split and make mask per group
mask1 = squeeze(sum(mask(1:length(dataset1),:,:),1)>mindatapoints);
if isfield(cfg,'searchspace')
    mask1=(mask1+pixsearchspace)==2;
end
if  ~isempty(dataset2)
    mask2 = squeeze(sum(mask(length(dataset1)+1:length(dataset1)+length(dataset2),:,:),1)>mindatapoints);
    if isfield(cfg,'searchspace')
        mask2=(mask2+pixsearchspace)==2;
    end
    maskcontrast=(mask1+mask2)==2;
end
clear mask

% Generate mask pictures
if logicalmask==2
    figure; imagesc(mask1); colormap(gray)
    print('-dtiff ','-r72','logicalmask1')
    close(gcf);
    if  ~isempty(dataset2)
        figure; imagesc(mask2); colormap(gray)
        print('-dtiff ','-r72','logicalmask2')
        close(gcf);
        figure; imagesc(maskcontrast); colormap(gray)
        print('-dtiff ','-r72','logicalmaskcontrast')
        close(gcf);
    end
end

% Generate raw fixation map pictures
if rawmaps==2
    for datasetnb=1:numberofdataset
        if datasetnb==1
            dataset=dataset1;
        ...
        for ii=1:length(dataset)
            poolraw=squeeze(matrixdurationtotal(ii+deb,:,:))+poolraw;
        end
        figure; imagesc(poolraw);
        if datasetnb==1
            print('-dtiff ','-r72','rawfix1')
        elseif datasetnb==2
            print('-dtiff ','-r72','rawfix2')
        end
        close(gcf);
    end
end
clear filtered_mat f_mat f_fil x y smoothpic gaussienne cumulfixdur cumulsaccadelength matrixduration summary matrixdurationtotal poolraw deb coordX coordY datatoload datafilenumber datasetnb fix ii nbfix tmp;
...
% Test dataset1
hwaitbar = waitbar(0,'Bootstrapping 1st dataset');
[h,prob,ci,stats]=ttest((smoothpictotal(1:length(dataset1),:,:)),mean(mean(mean((smoothpictotal(1:length(dataset1),:,:))))));
obstmapdataset1=squeeze(stats.tstat).*mask1;
obstmapdataset1(obstmapdataset1<0)=0;
obstfcedataset1=tfce2d(obstmapdataset1);
bootmaptotal=zeros(xSize,ySize,nboot);
B=1;
for b=1:nboot
    while length(unique(boot_table1(:,B))) < mindatapoints
        B=B+1;
    end
    [h,prob,ci,stats]=ttest(centeredgp1(boot_table1(:,B),:,:),0);
    tmp=squeeze(stats.tstat).*mask1;
    bootmaptotal(:,:,b)=tmp.*(tmp>0);
    B=B+1;
    waitbar(b / nboot)
end
close(hwaitbar)
boot_tfce_score = tfce2d( bootmaptotal);
clear bootmaptotal boot_gpe1 tmp h ci prob stats
hwaitbar = waitbar(0,'Sorting max values for 1st dataset');
for b=1:nboot
    tmp = squeeze(boot_tfce_score(:,:,b));
    max_tfceH0(b) = max(tmp(:));
    waitbar(b / nboot)
end
v=sort(max_tfceH0);
all_tfce_sig_dataset1= obstfcedataset1> v(round((1-p)*nboot));
dataset1mapfinal=all_tfce_sig_dataset1.*obstmapdataset1;
close(hwaitbar)
clear boot_tfce_score
...
if  ~isempty(dataset2)
    ...
    % Test dataset2
    hwaitbar = waitbar(0,'Bootstrapping 2nd dataset');
    [h,prob,ci,stats]=ttest((smoothpictotal(length(dataset1)+1:length(dataset1)+length(dataset2),:,:)),mean(mean(mean((smoothpictotal(length(dataset1)+1:length(dataset1)+length(dataset2),:,:))))));
    obstmapdataset2=squeeze(stats.tstat).*mask2;
    obstmapdataset2(obstmapdataset2<0)=0;
    obstfcedataset2=tfce2d(obstmapdataset2);
    bootmaptotal=zeros(xSize,ySize,nboot);
    B=1;
    for b=1:nboot
        while length(unique(boot_table2(:,B))) < mindatapoints
            B=B+1;
        end
        [h,prob,ci,stats]=ttest(centeredgp2(boot_table2(:,B),:,:),0);
        tmp= squeeze(stats.tstat).*mask2;
        bootmaptotal(:,:,b)=tmp.*(tmp>0);
        B=B+1;
        waitbar(b / nboot)
    end
    close(hwaitbar)
    boot_tfce_score = tfce2d( bootmaptotal);
    clear bootmaptotal boot_gpe2 tmp h ci prob stats
    hwaitbar = waitbar(0,'Sorting max values for 2nd dataset');
    for b=1:nboot
        tmp = squeeze(boot_tfce_score(:,:,b));
        max_tfceH0(b) = max(tmp(:));
        waitbar(b / nboot)
    ...
    v=sort(max_tfceH0);
    all_tfce_sig_dataset2= obstfcedataset2> v(round((1-p)*nboot));
    dataset2mapfinal=all_tfce_sig_dataset2.*obstmapdataset2;
    close(hwaitbar)
    clear boot_tfce_score 
    
    % Test contrast
    hwaitbar = waitbar(0,'Bootstrapping contrast');
    if twosampletest==1 % twosampletest: 1=paired or 2=independant
        [h,prob,ci,stats]=ttest(smoothpictotal(1:length(dataset1),:,:),smoothpictotal(length(dataset1)+1:length(dataset1)+length(dataset2),:,:));
        obstmapcontrast=squeeze(stats.tstat).*maskcontrast;
        ...
        clear obstmapcontrastpos obstmapcontrastneg
        bootmaptotal=zeros(xSize,ySize,nboot); 
        bootmaptotalpos=zeros(xSize,ySize,nboot);
        bootmaptotalneg=zeros(xSize,ySize,nboot);
        B = 1;
        for b=1:nboot
            while length(unique(boot_table1(:,B))) < mindatapoints
                B=B+1;
            end
           [h,prob,ci,stats]=ttest(centeredgp1(boot_table1(:,B),:,:),centeredgp2(boot_table1(:,B),:,:));
            bootmaptotal(:,:,b) = squeeze(stats.tstat).*maskcontrast;
            bootmaptotalpos(:,:,b)=bootmaptotal(:,:,b).*(bootmaptotal(:,:,b)>0);
            bootmaptotalneg(:,:,b)=bootmaptotal(:,:,b).*(bootmaptotal(:,:,b)<0);
            waitbar(b / nboot)
            B=B+1;
        end
        clear h prob ci stats
    else
        [h,prob,ci,stats]=ttest2((smoothpictotal(1:length(dataset1),:,:)),(smoothpictotal(length(dataset1)+1:length(dataset1)+length(dataset2),:,:)),p,'both','unequal',1);
        obstmapcontrast=squeeze(stats.tstat).*maskcontrast;
        obstmapcontrastpos=obstmapcontrast.*(obstmapcontrast>0);
        obstmapcontrastneg=obstmapcontrast.*(obstmapcontrast<0);
        obstfcecontrastpos=tfce2d(obstmapcontrastpos);
        obstfcecontrastneg=tfce2d(-obstmapcontrastneg);    
        clear obstmapcontrastpos obstmapcontrastneg
        bootmaptotal=zeros(cfg.xSize,cfg.ySize,nboot);
        bootmaptotalpos=zeros(cfg.xSize,cfg.ySize,nboot);
        bootmaptotalneg=zeros(cfg.xSize,cfg.ySize,nboot);
        B1 = 1; B2 = 1;
        for b=1:nboot
            while length(unique(boot_table1(:,B1))) < mindatapoints
                B1=B1+1;
            end
            while length(unique(boot_table2(:,B2))) < mindatapoints
                B2=B2+1;
            end
            [h,prob,ci,stats]=ttest2(centeredgp1(boot_table1(:,B1),:,:),centeredgp2(boot_table2(:,B2),:,:),p,'both','unequal',1);
            bootmaptotal(:,:,b) = squeeze(stats.tstat).*maskcontrast;
            bootmaptotalpos(:,:,b)=bootmaptotal(:,:,b).*(bootmaptotal(:,:,b)>0);
            bootmaptotalneg(:,:,b)=bootmaptotal(:,:,b).*(bootmaptotal(:,:,b)<0);
            waitbar(b / nboot)
    ...
    close(hwaitbar)
    clear bootmaptotal boot_gpe1 boot_gpe2 smoothpictotal
    boot_tfce_score_pos = tfce2d(bootmaptotalpos); clear bootmaptotalpos;
    boot_tfce_score_neg = tfce2d(-bootmaptotalneg); clear bootmaptotalneg;
    hwaitbar = waitbar(0,'Sorting max values for contrast');
    for b=1:nboot
        tmppos = squeeze(boot_tfce_score_pos(:,:,b));
        max_tfceH0_pos(b) = max(tmppos(:));
        tmpneg = squeeze(boot_tfce_score_neg(:,:,b));
        max_tfceH0_neg(b) = max(tmpneg(:));
        waitbar(b / nboot)
    end
    vpos=sort(max_tfceH0_pos);
    vneg=sort(max_tfceH0_neg);
    all_tfce_sig_contrast_pos= obstfcecontrastpos> vpos(round((1-p/2)*nboot));
    all_tfce_sig_contrast_neg= obstfcecontrastneg> vneg(round((1-p/2)*nboot));
    all_tfce_sig_contrast=(all_tfce_sig_contrast_pos+all_tfce_sig_contrast_neg)>0;
    contrastmapfinal=all_tfce_sig_contrast.*obstmapcontrast;
    close(hwaitbar)
    clear boot_tfce_score boot_tfce_score_pos boot_tfce_score_neg centeredgp1 centeredgp2 centeredallgroups stats ci
...
clear smoothpictotal prob mask mask1 mask2
